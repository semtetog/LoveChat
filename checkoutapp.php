...
<head>
    ...
    <title>Pagamento Seguro - Plano Full Vitalício - Love Chat</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; } html { scroll-behavior: smooth; font-size: 16px;}
        ...
        .payment-option h2 i.fa-credit-card { color: var(--text-primary); font-size: 1.2em; margin-right: 0.2em; }
        ...
        .toast-notification { background: var(--dark); color: var(--text-primary); padding: 1rem 1.5rem; border-radius: var(--border-radius-md); box-shadow: 0 6px 18px rgba(0,0,0,0.4); z-index: 1; opacity: 0; transition: all 0.4s cubic-bezier(0.2, 0.8, 0.2, 1.1); border: 1px solid var(--border-color); display: flex; align-items: center; gap: var(--space-md); min-width: 300px; border-left-width: 5px; }
        ...
        <main class="checkout-card" id="checkoutCardContent">
            <div class="card-content">
                <header class="checkout-header">
                    ...
                    <h1>Plano Full Vitalício</h1>
                    <p class="price">Valor Total: <strong>R$ 500,00</strong></p>
                </header>
                ...
                <section class="payment-option">
                    ...
                    <div class="qr-code-wrapper">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=190x190&data=PlaceholderPixKey" alt="QR Code Pix para Plano Full Vitalício" onerror="this.style.display='none'; this.nextElementSibling.textContent='Erro ao carregar QR Code. Use a chave abaixo.'">
                    ...
                    <div class="pix-key-wrapper">
                        ...
                        <div class="pix-key-input-group">
                            <input type="text" id="pixKeyInput" value="99.999.999/0001-99" readonly>
                            <button id="copyPixKeyBtn" data-pix-key="99999999000199">
                ...
                <section class="payment-option">
                    ...
                    <a href="https://wa.me/5534992981424?text=Olá,%20gostaria%20de%20pagar%20o%20Plano%20Full%20Vitalício%20com%20cartão." target="_blank" class="btn btn-whatsapp">
        ...
        document.addEventListener('DOMContentLoaded', () => {
           ...
           if (confirmButton && statusDiv && checkoutCardContent && backButton) {
               confirmButton.addEventListener('click', function() {
                   ...
                   const paymentData = {
                       userId: 123, // Valor de exemplo
                       packageName: "Plano Full Vitalício", // Valor de exemplo
                       leads: 800, // Valor de exemplo
                       price: 500.00, // Valor de exemplo
                       paymentMethod: 'pix_reported'
                   };

                   // SIMULAÇÃO DE CHAMADA API: O código abaixo simula o envio dos dados para um servidor.
                   // Em um ambiente real, você substituiria isso por uma chamada `fetch` ao seu backend.
                   new Promise((resolve, reject) => {
                       console.log("Simulando notificação de pagamento:", paymentData);
                       // Simula um atraso de rede de 1.5 segundos
                       setTimeout(() => {
                           // Para testar um cenário de erro, descomente a linha abaixo:
                           // reject(new Error('Falha na comunicação com o servidor.'));
                           resolve({ success: true, message: 'Notificação enviada com sucesso.' });
                       }, 1500);
                   })
                   .then(result => {
                       if (!result.success) throw new Error(result.message || 'Erro ao notificar pagamento.');

                       statusDiv.className = 'payment-status success';
                       statusDiv.innerHTML = '<i class="fas fa-check-circle"></i> Notificação recebida! Após a confirmação, seu acesso será liberado.';
                       statusDiv.style.display = 'block';

                       // Esconde as outras seções do formulário para focar na mensagem de sucesso
                       Array.from(checkoutCardContent.children).forEach(child => {
                           if (child && child !== statusDiv && child !== backButton) {
                       ...
                       if(backButton) { 
                           ...
                           backButton.style.marginTop = 'var(--space-lg)'; 
                           backButton.style.opacity = '1';
                       }
                       statusDiv.scrollIntoView({ behavior: 'smooth', block: 'center' });

                   }).catch(error => {
                       console.error('Erro na simulação de confirmação Pix:', error);
                       ...
                       statusDiv.style.display = 'block';
                       this.disabled = false;
                       this.innerHTML = '<i class="fas fa-check-double"></i> Já Paguei com Pix';
                   });
               });
           }
        });
