...
<head>
    ...
    * {
    ...
    .sidebar {
        ...
        box-shadow: 0 8px 32px rgba(0, 0, 0, 0.4), inset -1px 0 0 rgba(255, 255, 255, 0.05);
    ...
    .user-profile:hover {
        ...
        box-shadow: 0 6px 24px rgba(255, 0, 127, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.15);
    }
    ...
    .menu-item.active {
        ...
        border-color: rgba(255, 0, 127, 0.3);
        box-shadow: 0 4px 16px rgba(255, 0, 127, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.1);
    }
    ...
    .btn i {
     ...
     /* Estilos Base Botão (Dark Theme) - GLASSMORPHISM */
    .btn {
    ...
    .btn:hover {
        ...
        box-shadow: 0 6px 20px rgba(255, 0, 127, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.15);
    }

    /* Botões Específicos - GLASSMORPHISM */
    .btn-edit {
    ...
    .btn-details {
        background: rgba(252, 92, 172, 0.15);
        backdrop-filter: blur(10px);
        -webkit-backdrop-filter: blur(10px);
        color: var(--secondary);
        border-color: rgba(252, 92, 172, 0.3);
    }
    .btn-details:hover {
        background: rgba(252, 92, 172, 0.3);
        color: white;
        border-color: var(--secondary);
        box-shadow: 0 6px 20px rgba(252, 92, 172, 0.4);
    }
    ...
    .feed-item:hover {
        box-shadow: 0 12px 40px rgba(0, 0, 0, 0.4), 0 0 0 1px rgba(255, 0, 127, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.15);
    ...
    .feed-item.unread {
        ...
        backdrop-filter: blur(20px) saturate(180%);
        -webkit-backdrop-filter: blur(20px) saturate(180%);
        box-shadow: 0 8px 32px rgba(255, 0, 127, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.15);
    }
    .feed-item.unread:hover {
        border-left-color: var(--primary-dark);
        background: rgba(255, 0, 127, 0.25);
        box-shadow: 0 12px 40px rgba(255, 0, 127, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.2);
    }
    ...
    .modal-content {
        ...
        border: 1px solid rgba(255, 255, 255, 0.1);
        animation: modalContentSlideIn 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.1);
        position: relative;
        overflow: hidden;
        display: flex;
        flex-direction: column;
    ...
    .modal-content::before {
        ...
        right: 0;
        height: 1px;
        background: linear-gradient(90deg, transparent, rgba(255, 0, 127, 0.5), transparent);
    }
    ...
    .form-control:focus {
        outline: none;
        border-color: var(--primary);
        box-shadow: 0 0 0 3px rgba(255, 0, 127, 0.2), inset 0 2px 4px rgba(0, 0, 0, 0.2), 0 1px 0 rgba(255, 255, 255, 0.1);
        background: rgba(255, 0, 127, 0.1);
...
/* ========== Badge Circular Indicador (Sidebar) - REFINADO ========== */

/* Garante que o item de menu seja o container de posicionamento */
.menu-item[data-section="whatsapp_pending"] {
    position: relative;
}

/* Estilo do Badge Circular */
.menu-item .badge#pending-wa-count-badge {
    /* Reset e Dimensionamento */
    box-sizing: border-box; /* Borda/Padding não aumentam o tamanho total */
    display: inline-flex; /* Usa flexbox para alinhar o conteúdo interno */
    width: 26px;          /* Largura - Aumentado um pouco mais */
    height: 26px;         /* Altura igual à largura */
    padding: 0;           /* Sem padding interno */
    border-radius: 50%;   /* Círculo perfeito */

    /* Alinhamento do número DENTRO do círculo */
    align-items: center;     /* Alinha verticalmente no centro */
    justify-content: center; /* Alinha horizontalmente no centro */
    text-align: center;      /* Garante alinhamento do texto */

    /* Estilo Visual */
    background-color: var(--warning);   /* Fundo amarelo */
    color: #333; /* Cor do número mais escura para melhor contraste */
    font-size: 0.8rem;           /* Tamanho da fonte */
    font-weight: 600;             /* Peso da fonte (ajuste se preferir) */
    line-height: 1;               /* Importante para centralização vertical do texto */
    box-shadow: 0 1px 2px rgba(0,0,0,0.2); /* Sombra mais sutil */
    border: none;                 /* Remove a borda anterior para visual mais clean */

    /* Posicionamento */
    position: absolute;
    right: 10px;                /* Distância da direita - Diminuído um pouco */
    top: 50%;
    transform: translateY(-50%); /* Centralização vertical */
    z-index: 2;

    /* Remove margens que podem interferir */
    margin: 0;
}

/* Garante que o número (se estiver em span interno) não tenha estilos extras */
.menu-item .badge#pending-wa-count-badge span {
    display: block;
    margin: 0;
    padding: 0;
    line-height: 1; /* Garante herança */
}


@keyframes subtle-glow {
    0%, 100% { box-shadow: 0 0 3px rgba(255, 204, 0, 0.5); }
    50% { box-shadow: 0 0 6px rgba(255, 204, 0, 0.8); }
}
.menu-item .badge#pending-wa-count-badge {
...
/* ========== ESTILOS ADICIONAIS: RANK E SALDOS ========== */

/* ========== ESTILOS RANK MELHORADOS (v3 - Foco em Alinhamento Vertical Padrão) ========== */
...
/* --- Alinhamento e Padding Padronizados para TODAS as Células do Rank --- */
#rankTable td {
    padding-top: var(--space-md);    /* Padding superior consistente */
    padding-bottom: var(--space-md); /* Padding inferior consistente */
    vertical-align: middle;          /* <<< FORÇA alinhamento vertical ao meio (padrão de tabela) */
    /* Removemos display: flex daqui */
}

/* --- Estilos específicos por coluna (principalmente horizontal) --- */
...
#rankTable .rank-cell {
    ...
    min-width: 110px;
    border-right: none;
    /* Não precisa de display: flex aqui, vamos controlar os elementos internos */
    white-space: nowrap; /* Impede que o número e o badge quebrem linha */
}

/* Remove a linha vertical entre a 1ª e 2ª coluna */
#rankTable .rank-cell + td {
    border-left: none !important;
}

/* Célula Usuário (Segunda Coluna) */
#rankTable .user-info-cell {
    /* A div interna já usa flex para alinhar avatar e nome */
    vertical-align: middle; /* Garante alinhamento mesmo dentro do flex */
}

/* Célula Ganhos (Terceira Coluna) */
#rankTable td.rank-earnings {
    text-align: right; /* Alinha o texto (R$) à direita */
    padding-right: var(--space-lg); /* Espaço à direita */
    font-size: 1rem;
    vertical-align: middle; /* Reforça alinhamento */
}
#rankTable .rank-earnings .rank-value,
#rankTable .rank-earnings .rank-edit-input {
     text-align: right; /* Garante alinhamento do texto dentro do span/input */
}

/* Célula Ações (Última Coluna) */
#rankTable td:last-child {
    text-align: center; /* Centraliza o conteúdo (div.action-buttons) */
    padding-right: var(--space-lg); /* Adiciona padding direito */
    min-width: 120px; /* Largura mínima para os botões */
    vertical-align: middle; /* Reforça alinhamento */
}
#rankTable td:last-child .action-buttons {
     justify-content: center; /* Centraliza os botões DENTRO da div flex */
     display: inline-flex; /* Garante que a div não ocupe 100% da célula */
}

/* --- Estilo das Linhas (Tr) --- */
#rankTable tbody tr {
    transition: background-color 0.2s ease, border-left-color 0.3s ease;
    border-left: 4px solid transparent; /* Borda para destaque Top 3 */
    border-bottom: 1px solid var(--border-color); /* Borda inferior consistente */
}
#rankTable tbody tr:last-child {
    border-bottom: none; /* Remove da última linha */
}
#rankTable tbody tr:hover {
    background-color: var(--gray-light);
}

/* --- Destaque Linhas Top 3 --- */
#rankTable tbody tr.rank-1 {
    background-color: rgba(255, 215, 0, 0.05);
    border-left-color: var(--rank-gold);
}
#rankTable tbody tr.rank-2 {
    background-color: rgba(192, 192, 192, 0.05);
    border-left-color: var(--rank-silver);
}
#rankTable tbody tr.rank-3 {
    background-color: rgba(205, 127, 50, 0.05);
    border-left-color: var(--rank-bronze);
}
#rankTable tbody tr.rank-1:hover,
#rankTable tbody tr.rank-2:hover,
#rankTable tbody tr.rank-3:hover {
     background-color: rgba(255, 255, 255, 0.08);
}

/* --- Estilo do Número da Posição --- */
.rank-position {
    font-size: 1.1rem;
    font-weight: 700;
    color: var(--text-secondary);
    ...
    vertical-align: middle; /* Alinha com o badge */
    margin-right: var(--space-md); /* Espaço entre número e badge */
}

/* --- Novo Estilo Base para Badges do Rank --- */
.rank-badge-v2 {
    ...
    flex-shrink: 0;
    vertical-align: middle; /* Alinha com o número */
}
.rank-badge-v2 i {
    line-height: 1;
}
...
/* --- Badge Ouro (#1) --- */
.rank-badge-v2.gold {
    background: linear-gradient(145deg, #fff0a3, #ffc107);
    border-color: #d4a000;
    color: #5e4500;
...
/* --- Badge Bronze (#3) --- */
.rank-badge-v2.bronze {
    background: linear-gradient(145deg, #f4c8a2, #cd7f32);
    border-color: #a05d21;
    color: #50300a;
}
...
@keyframes gold-glow {
    0%, 100% { box-shadow: 0 3px 5px rgba(0, 0, 0, 0.3), inset 0 1px 1px rgba(255, 255, 255, 0.15), inset 0 -1px 1px rgba(0, 0, 0, 0.2), 0 0 8px 2px rgba(255, 215, 0, 0.4); border-color: #d4a000; }
    50% { box-shadow: 0 3px 8px rgba(0, 0, 0, 0.4), inset 0 1px 1px rgba(255, 255, 255, 0.2), inset 0 -1px 1px rgba(0, 0, 0, 0.3), 0 0 18px 5px rgba(255, 223, 77, 0.6); border-color: #ffe057; }
}

/* --- Coluna Usuário e Ganhos --- */
#rankTable .user-info-cell { /* Ajuste para alinhar avatar e nome */
     vertical-align: middle;
}
#rankTable .user-name {
    font-weight: 600;
...
/* Destaque nos ganhos do Top 3 */
#rankTable tr.rank-1 .rank-earnings .rank-value { color: var(--rank-gold); text-shadow: 0 0 5px rgba(255, 215, 0, 0.5); }
#rankTable tr.rank-2 .rank-earnings .rank-value { color: var(--rank-silver); text-shadow: 0 0 5px rgba(192, 192, 192, 0.5); }
#rankTable tr.rank-3 .rank-earnings .rank-value { color: var(--rank-bronze); text-shadow: 0 0 5px rgba(205, 127, 50, 0.5); }

/* --- Ajustes Botões de Ação --- */
#rankTable .rank-edit-input {
    width: 110px;
    margin-left: var(--space-sm);
}
.rank-editing td { background-color: rgba(255, 204, 0, 0.1); }
.rank-edit-btn { margin-left: var(--space-sm); opacity: 0.6; transition: opacity 0.2s ease; } .rank-edit-btn:hover { opacity: 1; }
.rank-edit-input { display: inline-block; width: 100px; padding: 4px 8px; font-size: var(--font-size-sm); background-color: var(--gray-dark); border-color: var(--warning); text-align: right; margin-left: 5px; }
    /* --- Gerenciamento de Saldos --- */
    .balance-date-navigation .flatpickr-input { /* Estilo específico para o input de data */
         width: auto;
         min-width: 150px;
         background-color: var(--gray);
     ...
     .balance-table .pix-key { font-family: monospace; font-size: var(--font-size-xs); color: var(--text-secondary); word-break: break-all; display: inline-block; /* Evita quebra estranha */ max-width: 150px; /* Limita largura */ overflow: hidden; text-overflow: ellipsis; white-space: nowrap; vertical-align: bottom; }
     .balance-table .balance-value-cell { font-weight: 600; /* Saldo em negrito */ }
     .balance-table .balance-proof-cell { color: var(--text-secondary); } /* Total comprovantes mais sutil */
     .balance-table tr.balance-paid td { opacity: 0.7; /* Linhas pagas mais apagadas */ }
     .balance-table tr.balance-paid .badge-success { background-color: var(--gray-light); color: var(--text-secondary); border: 1px dashed var(--border-color); } /* Badge "Sim" pago mais sutil */
     .balance-table tr.balance-unpaid .balance-value-cell { color: var(--success); } /* Destaca saldo pendente */
     .balance-table .btn-warning { /* Botão desmarcar */ background-color: var(--warning-light); color: var(--warning); border-color: var(--warning); }
     .balance-table .btn-warning:hover:not(:disabled) { background-color: var(--warning); color: var(--dark); }

    /* --- Botão Zerar Saldo --- */
    .btn-reset-balance { background-color: var(--warning-light); color: var(--warning); border-color: var(--warning); }
    .btn-reset-balance:hover:not(:disabled) { background-color: var(--warning); color: var(--dark); border-color: var(--warning); }
    .btn-reset-balance i.fa-times { font-size: 0.7em; position: relative; top: -0.2em; left: -0.3em; opacity: 0.8; }

    /* --- Tabela de Usuários (Saldo Hoje) --- */
    .user-balance-cell { font-weight: 600; text-align: right; padding-right: var(--space-lg) !important; } /* Alinha saldo à direita */

/* ========== FLATICKR DARK THEME ADJUSTMENTS (REFORÇADO) ========== */

/* Container Principal do Calendário */
.flatpickr-calendar {
    background: var(--gray-dark) !important;  /* Fundo principal */
    border-color: var(--border-color) !important; /* Borda sutil */
    box-shadow: var(--shadow-lg) !important;    /* Sombra */
    color: var(--text-primary) !important;     /* Cor do texto padrão */
    border-radius: var(--border-radius-md) !important; /* Bordas arredondadas */
}

/* Cabeçalho (Mês e Ano) */
.flatpickr-months {
    background-color: var(--dark); /* Fundo ligeiramente diferente */
    border-bottom: 1px solid var(--border-color);
}
.flatpickr-months .flatpickr-month {
    color: var(--text-primary) !important;
    fill: var(--text-primary) !important; /* Cor das setas internas do mês/ano */
    height: 40px; /* Ajuste altura se necessário */
    line-height: 40px;
}
.flatpickr-months .flatpickr-prev-month,
.flatpickr-months .flatpickr-next-month {
    fill: var(--text-secondary) !important; /* Cor das setas de navegação */
    color: var(--text-secondary) !important; /* Cor do texto (se houver) */
    top: 10px; /* Ajusta posição vertical das setas */
    padding: 5px;
    height: 24px;
    width: 24px;
}
...
.flatpickr-months .flatpickr-next-month:hover {
    ...
    background: rgba(255, 255, 255, 0.05) !important; /* Fundo sutil no hover */
}

/* Dropdown de Mês e Input de Ano */
...
.numInputWrapper { /* Container do input de ano e setinhas */
    background: var(--gray) !important;
    border-radius: var(--border-radius-sm);
}
.numInputWrapper input.numInput { /* Input do ano */
    background: transparent !important;
    color: inherit !important;
    border: none !important;
}
.numInputWrapper span { /* Setinhas do ano */
    border-color: var(--border-color-light) !important;
    border-left: 1px solid var(--border-color-light) !important; /* Linha separadora */
}
.numInputWrapper span:hover {
    background: var(--gray-light) !important;
}
.numInputWrapper span::after { /* Cor das setinhas do ano */
...
/* Dias da Semana */
.flatpickr-weekdays {
    background: var(--darkest) !important; /* Fundo bem escuro */
    border-bottom: 1px solid var(--border-color);
}
...
.flatpickr-day {
    color: var(--text-primary) !important;
    border-color: transparent !important; /* Remove borda padrão */
    line-height: 36px; /* Ajusta altura */
    height: 36px;
    font-weight: 500;
}
.flatpickr-day:hover,
...
.flatpickr-day.selected.inRange, /* Para seleção de range */
...
.flatpickr-day.today {
    border-color: var(--secondary) !important; /* Borda para o dia atual */
...
.flatpickr-day.prevMonthDay, /* Dias do mês anterior/seguinte */
...
/* Opcional: Barra de Tempo (se habilitada) */
.flatpickr-time {
...
/* ========== FIM FLATICKR DARK THEME ========== */
#balanceManagerTableBody {
    transition: opacity 0.3s ease-in-out; /* Aplica transição na opacidade */
    opacity: 1; /* Estado padrão visível */
}

#balanceManagerTableBody.loading-content {
    opacity: 0; /* Estado quando está carregando/esmaecendo */
    /* Opcional: Evitar que o usuário clique enquanto carrega */
    /* pointer-events: none; */
}

/* Ajuste opcional para a mensagem de 'Carregando...' se ainda a usar */
#balanceManagerTableBody.loading-content tr td i.fa-spinner {
    /* Mantém o spinner visível mesmo com opacidade baixa no tbody, se necessário */
    /* opacity: 1 !important; */
}

/* ========== ESTILOS PARA STATUS BADGE (WhatsApp Pendente) ========== */
.status-badge-wa {
    display: inline-flex; /* Para alinhar ícone e texto */
    align-items: center; /* Alinha verticalmente */
    padding: 4px 10px; /* Espaçamento interno */
    border-radius: var(--border-radius-xl); /* Bordas arredondadas */
    font-size: var(--font-size-xs); /* Tamanho da fonte pequeno */
    font-weight: 600; /* Peso da fonte */
    /* Removido margin-right para colocar no TD */
    white-space: nowrap; /* Impede quebra de linha */
    border: 1px solid transparent; /* Borda sutil */
    line-height: 1.4; /* Ajuste de linha */
    vertical-align: middle; /* Alinha verticalmente com outros elementos na célula */
}
.status-badge-wa i {
    margin-right: 5px; /* Espaço entre ícone e texto */
    font-size: 0.9em; /* Tamanho do ícone relativo ao texto */
}

/* Cores específicas dos badges */
.status-badge-wa.status-pending { /* Amarelo/Laranja */
    background-color: var(--warning-light);
    color: var(--warning);
    border-color: rgba(255, 204, 0, 0.3);
}
.status-badge-wa.status-aguardando_resposta { /* Azul */
    background-color: var(--info-light);
    color: var(--info);
    border-color: rgba(0, 153, 255, 0.3);
}
.status-badge-wa.status-processing { /* Rosa Secundário (Pode escolher outra cor info) */
    background-color: var(--secondary-light);
    color: var(--secondary);
    border-color: rgba(252, 92, 172, 0.3);
}
.status-badge-wa.status-approved { /* Verde */
    background-color: var(--success-light);
    color: var(--success);
    border-color: rgba(0, 204, 102, 0.3);
}
.status-badge-wa.status-rejected { /* Vermelho */
    background-color: var(--danger-light);
    color: var(--danger);
    border-color: rgba(255, 51, 51, 0.3);
}

/* Estilo para a célula que contém o badge e o select */
#pending-whatsapp-table td.status-cell {
    display: flex; /* Organiza badge e select lado a lado */
    align-items: center; /* Alinha verticalmente */
    gap: var(--space-sm); /* Espaço entre badge e select */
}

/* ========== ESTILOS PARA FILTROS (WhatsApp Pendente) ========== */
.whatsapp-filter-controls {
    display: flex;
    flex-wrap: wrap; /* Quebra linha em telas menores */
    gap: var(--space-sm); /* Espaçamento entre botões */
    margin-bottom: var(--space-lg); /* Espaço abaixo dos filtros */
    padding-bottom: var(--space-md); /* Linha abaixo */
    border-bottom: 1px solid var(--border-color);
}
...
.filter-btn {
    ...
    transition: all 0.2s ease;
    border: 1px solid var(--border-color);
    background-color: var(--gray); /* Fundo padrão */
    color: var(--text-secondary); /* Cor padrão */
    display: inline-flex;
    align-items: center;
    gap: 5px;
    position: relative;
...
.filter-count-badge {
    ...
    position: absolute;
    top: -8px; /* Posição acima do botão */
    right: -8px; /* Posição à direita do botão */
    background-color: var(--gray-light); /* Cor de fundo do badge */
    color: var(--text-primary); /* Cor do texto do badge */
    border: 1px solid var(--border-color-light);
    font-size: 0.65rem; /* Tamanho da fonte bem pequeno */
    font-weight: 700;
    min-width: 18px; /* Largura mínima */
    height: 18px; /* Altura */
    line-height: 16px; /* Alinhar texto verticalmente */
    text-align: center;
    border-radius: 9px; /* Deixa quase circular */
    padding: 0 5px; /* Padding horizontal */
    box-shadow: var(--shadow-sm);
    z-index: 1; /* Fica acima do botão */
}

/* Mostra o badge quando ele tem conteúdo (classe 'show' adicionada via JS) */
...
/* Opcional: Cor diferente para o badge no botão ativo */
...
.filter-btn.active {
    background-color: var(--gray); /* Mantém fundo cinza padrão */
    color: var(--lighter); /* Texto um pouco mais claro para destaque */
    border: 2px solid var(--primary); /* Borda rosa mais grossa */
    /* Ajusta padding para compensar a borda mais grossa */
    padding: 5px 13px; /* 1px a menos no vertical/horizontal */
    font-weight: 600;
    box-shadow: none; /* Remove sombra */
}

/* Opcional: Leve escurecida no hover do botão ativo para feedback */
.filter-btn.active:hover {
    background-color: var(--gray-light);
}
/* Ajuste para que o dropdown não ocupe toda a largura na célula flex */
#pending-whatsapp-table td.status-cell .feed-item-status {
    flex-grow: 1; /* Faz o select ocupar o espaço restante, se necessário */
    min-width: 150px; /* Garante uma largura mínima */
}


/* ========== ESTILOS PARA LEGENDA WHATSAPP ========== */
.whatsapp-status-legend {
    background-color: rgba(0,0,0,0.15); /* Fundo semi-transparente sutil */
    border: 1px solid var(--border-color);
    border-radius: var(--border-radius-md);
    padding: var(--space-md);
    margin-top: var(--space-lg); /* Espaço acima da tabela */
    font-size: var(--font-size-sm);
}

.whatsapp-status-legend h4 {
    font-size: var(--font-size-base);
    font-weight: 600;
    color: var(--text-primary);
    margin-bottom: var(--space-sm);
...
.whatsapp-status-legend h4 i { /* Estilo para o ícone do título */
    color: var(--info);
}

.whatsapp-status-legend ul {
    list-style: none;
    padding: 0;
    margin: 0;
    display: grid; /* Organiza em colunas se couber */
    /* Tenta 2 colunas, mínimo de 280px cada */
    grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
    gap: var(--space-sm) var(--space-lg); /* Espaço vertical e horizontal */
}

.whatsapp-status-legend li {
    display: flex;
...
.whatsapp-status-legend li div { /* Container do texto */
    flex-grow: 1; /* Ocupa espaço restante */
}

.whatsapp-status-legend .legend-color {
    display: inline-block;
    width: 14px;
    height: 14px;
    border-radius: 4px; /* Quadrado mais arredondado */
    flex-shrink: 0; /* Não encolhe */
    margin-top: 4px; /* Alinha melhor com a primeira linha de texto */
    border: 1px solid rgba(255, 255, 255, 0.15); /* Borda sutil */
    box-shadow: inset 0 1px 2px rgba(0,0,0,0.2); /* Sombra interna sutil */
}

/* Cores dos indicadores da legenda */
.legend-color.status-pending { background-color: var(--warning); border-color: var(--warning); }
.legend-color.status-aguardando_resposta { background-color: var(--info); border-color: var(--info); }
.legend-color.status-processing { background-color: var(--secondary); border-color: var(--secondary); }
.legend-color.status-approved { background-color: var(--success); border-color: var(--success); }
.legend-color.status-rejected { background-color: var(--danger); border-color: var(--danger); }

.whatsapp-status-legend strong {
    color: var(--text-primary);
    font-weight: 600;
}

/* === Cursor de Mãozinha para Linhas Clicáveis (WhatsApp Pendente) === */
#pending-whatsapp-table tbody tr[data-user-id] {
    /* Aplica o cursor de mãozinha à linha inteira */
    cursor: pointer;
}

/* Garante que elementos interativos DENTRO da linha mantenham seus cursores normais */
#pending-whatsapp-table tbody tr[data-user-id] button,
#pending-whatsapp-table tbody tr[data-user-id] a,
#pending-whatsapp-table tbody tr[data-user-id] select,
#pending-whatsapp-table tbody tr[data-user-id] .status-badge-wa, /* Badge também pode não ser clicável para detalhes */
#pending-whatsapp-table tbody tr[data-user-id] .feed-status-select /* O select de status */ {
    /* Retorna o cursor padrão para esses elementos específicos */
    cursor: auto; /* ou 'pointer' para botões/links, 'default' para selects, se necessário */
    /* Na prática, o navegador geralmente já faz isso, mas é bom garantir */
}

/* Especificamente para botões e links DENTRO da linha, manter o pointer */
#pending-whatsapp-table tbody tr[data-user-id] button,
#pending-whatsapp-table tbody tr[data-user-id] a.btn {
    cursor: pointer;
}

/* === Cursor de Mãozinha para Linhas Clicáveis (Gerenciar Usuários) === */
#usersTable tbody tr[data-user-id] {
    cursor: pointer;
}

/* Resetar cursor para elementos interativos DENTRO da linha Usuários */
#usersTable tbody tr[data-user-id] button,
#usersTable tbody tr[data-user-id] a,
#usersTable tbody tr[data-user-id] .badge { /* Inclui badges de status/tipo */
    cursor: auto;
}
#usersTable tbody tr[data-user-id] button,
#usersTable tbody tr[data-user-id] a.btn {
    cursor: pointer;
}


/* === Cursor de Mãozinha para Linhas Clicáveis (Rank) === */
#rankTable tbody tr[data-user-id] {
    cursor: pointer;
}

/* Resetar cursor para elementos interativos DENTRO da linha Rank */
#rankTable tbody tr[data-user-id] button,
#rankTable tbody tr[data-user-id] a,
#rankTable tbody tr[data-user-id] input, /* Input de edição */
#rankTable tbody tr[data-user-id] .rank-badge-v2, /* Badge de medalha */
#rankTable tbody tr[data-user-id] .rank-edit-input {
    cursor: auto;
}
#rankTable tbody tr[data-user-id] button,
#rankTable tbody tr[data-user-id] a.btn {
    cursor: pointer;
}

/* Remove o cursor pointer do avatar dentro da user-info-cell (se não desejado) */
/* .user-info-cell img.user-avatar-sm {
    cursor: default;
} */


/* === CORREÇÕES PARA BOTÕES EM #confirmationModal === */

/* Garante que o footer permita que os botões se dimensionem */
#confirmationModal .modal-footer {
    ...
    min-height: 60px; /* Altura mínima para o footer, se necessário */
}

/* Estilos Essenciais para AMBOS os botões dentro do footer específico */
#confirmationModal .modal-footer .btn {
    /* Layout e Alinhamento */
    display: inline-flex !important; /* Garante flex layout para ícone+texto */
    align-items: center !important; /* Alinha verticalmente */
    justify-content: center !important; /* Alinha horizontalmente */
    vertical-align: middle; /* Ajuda no alinhamento com outros elementos */

    /* Dimensionamento e Espaçamento */
    padding: 8px 18px !important; /* Padding generoso (ajuste se necessário) */
    height: auto !important; /* Altura automática baseada no conteúdo */
    width: auto !important; /* Largura automática */
    min-width: 100px; /* Largura mínima para evitar botões muito estreitos */

    /* Texto */
    font-size: var(--font-size-sm) !important; /* Tamanho da fonte */
    line-height: 1.5 !important; /* Altura da linha adequada */
    ...
    /* Outros */
    cursor: pointer;
    transition: all 0.2s ease !important; /* Garante animação suave */
    outline: none !important; /* Remove outline padrão */
}

/* Estilo Específico para o Botão CANCELAR (Secundário) */
...
#confirmationModal .modal-footer .btn-secondary:hover {
    background-color: var(--gray);
    border-color: var(--border-color-light);
    transform: translateY(-1px); /* Pequeno efeito no hover */
}

/* Estilo Específico para o Botão CONFIRMAR (a classe btn-danger/etc é adicionada via JS) */
#confirmationModal .modal-footer #confirmActionButton {
    /* Estilos de cor/fundo são aplicados pela classe dinâmica (btn-danger, etc) */
    /* Ex: .btn-danger { background-color: var(--danger); color: white; border-color: var(--danger); } */
}
#confirmationModal .modal-footer #confirmActionButton:hover {
     transform: translateY(-1px); /* Efeito hover genérico */
     /* O hover específico da classe (ex: .btn-danger:hover) pode adicionar mais efeitos */
}

/* Garante que o ícone dentro do botão tenha um espaço */
#confirmationModal .modal-footer .btn i {
    margin-right: 6px; /* Espaço entre ícone e texto */
    line-height: 1; /* Garante alinhamento do ícone */
}

/* === Estilos para Modal Detalhes do Usuário - Adicionais === */

/* Ajuste para nome e badge do rank no header */
.user-detail-header .user-detail-name {
    display: inline-block; /* Permite que o badge fique ao lado */
    vertical-align: middle;
    margin-right: var(--space-sm); /* Espaço antes do badge */
    margin-bottom: 0; /* Remove margem inferior padrão se houver */
}

/* Estilo para o badge de rank DENTRO do header do modal */
.user-detail-header .rank-badge-v2 {
    /* Herda de .rank-badge-v2, mas podemos ajustar o tamanho/alinhamento */
    width: 28px;           /* Um pouco menor */
    height: 28px;
    font-size: 1rem;       /* Ícone um pouco menor */
    vertical-align: middle;/* Alinha com o texto do nome */
    margin-left: 0;        /* Remove margem padrão se houver */
    /* Remove a animação de brilho se achar exagerado no modal */
    /* animation: none; */
}

/* Destaque para Saldo Devedor */
.user-detail-value.outstanding-balance {
    color: var(--danger) !important; /* Cor vermelha */
    font-weight: bold !important;   /* Negrito */
    background-color: rgba(255, 51, 51, 0.1) !important; /* Fundo vermelho claro */
    border: 1px solid rgba(255, 51, 51, 0.4) !important; /* Borda vermelha sutil */
    position: relative; /* Para o pseudo-elemento */
    padding-left: calc(var(--space-md) + 20px) !important; /* Espaço para ícone */
}

.user-detail-value.outstanding-balance::before {
    content: "\f071"; /* Código do ícone de aviso (exclamation-triangle) */
    font-family: "Font Awesome 6 Free"; /* Fonte correta */
    font-weight: 900; /* Peso para ícones sólidos */
    color: var(--danger); /* Cor do ícone */
    position: absolute;
    left: var(--space-md); /* Posição à esquerda */
    top: 50%;
    transform: translateY(-50%); /* Centraliza verticalmente */
    font-size: 1em; /* Tamanho do ícone */
}

/* Badge Saldo Devedor no Header (alternativa/complemento ao campo) */
.user-detail-status .badge-danger {
    background-color: var(--danger-light);
    color: var(--danger);
    border: 1px solid var(--danger); /* Borda mais visível */
}
    </style>
</head>
<body>
